...
global {
	...
	reflex live_policies {
		if social_dist = not(sd_trigg) { do social_distancing; sd_trigg <- not(sd_trigg); }
...
species people skills:[moving] {
	...
	reflex move {
 	...
 	reflex infect when:state="I" {
 		if quarantine and not(free_rider) {social_space <- location; target <- nil;}
...
experiment NewModel1 type: gui {
	output {
		display main {
			...
			event 'r' {ask simulation {do social_distancing;}}
		}
		display chart {
