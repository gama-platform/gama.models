...
species people skills:[moving] {
	...
	reflex follow_agenda when: agenda.keys one_matches (each = current_date.hour) and activity != agenda[current_date.hour]{
		activity <- agenda[current_date.hour];
		if free_rider or not social_dist{
			switch activity{
				match "home"{target <- any_location_in(home);}
				match "work"{target <- any_location_in(work);}
				match "eat"{target <- any_location_in(home);}
			}
		}else{
			switch activity{
				match "home"{target <- any_location_in(home);}
				match "work"{target <- any_location_in(home);}
				match "eat"{target <- any_location_in(home);}
			}
		}
	}
