...
experiment display_map_overlay {
	output {
		display landuse background: #lightgray {
			image file("../includes/background.png") refresh: false;
			species parcel aspect: land_use;
			
			overlay position: {10 #px, 10 #px} size: {200 #px, 200 #px} background: #black rounded: true {
				draw "LU" at: {20#px, 20#px} anchor: #top_left color: #white ;
				draw square(20 #px) at: {20 #px, 50#px} color: #darkgreen border: #black;		
				draw "BHK" at: {50 #px, 50#px} anchor: #left_center color: #white;		
			}
